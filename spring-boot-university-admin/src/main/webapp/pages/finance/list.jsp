...
<head>
    <meta charset="UTF-8">
    <title>缴费记录 - Spring Boot 版</title>
        ...
        <table class="data-table">
            ...
            <tbody>
                <c:if test="${empty tuitions}">
                    <tr>
                        <td colspan="7" class="empty-msg">暂无缴费记录，请登记缴费</td>
                    </tr>
                </c:if>
                <c:forEach items="${tuitions}" var="t">
                    <tr>
                        ...
                        <td>${t.studentNo}</td>
                        <td>${t.studentName}</td>
                        <!-- amountYuan 是 Tuition 实体中自定义的计算属性 -->
                        <td><fmt:formatNumber value="${t.amountYuan}" pattern="#,##0.00"/></td>
                        <td>${t.semester}</td>
                        ...
                        <td class="action-cell">
                            <a href="${pageContext.request.contextPath}/finance/delete?id=${t.id}"
                               ...
                               onclick="return confirm('确定要删除这笔缴费记录吗？')">删除</a>
                        </td>
                    </tr>
                </c:forEach>
            </tbody>
